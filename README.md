...
5. **Copy to storage class**: The S3 storage class to copy object versions into. As always with S3, [if in doubt, use Intelligent-Tiering](https://catalog.workshops.aws/awscff/en-US/playbooks/storage/s3/2-choosing-storage-class). See [the documentation](https://docs.aws.amazon.com/AmazonS3/latest/userguide/sc-howtoset.html) for more information.
...
Once the CloudFormation deployment is complete, review the S3 Batch Operations Jobs and their manifests using the S3 Console, then run the jobs. If using KMS encryption, ensure the role being used with this tool has the necessary permissions before running the jobs. Also see the section below on [Lambda concurrency reservations](#lambda-concurrency-reservations).
